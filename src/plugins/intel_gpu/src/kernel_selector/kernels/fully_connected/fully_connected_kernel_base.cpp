...
JitConstants FullyConnectedKernelBase::GetJitConstants(const fully_connected_params& params,
    ...
    if (params.compressed) {
        ...
        } else if (params.weights.GetDType() == WeightsType::UT2 || params.weights.GetDType() == WeightsType::UT1_5) {
            jit.AddConstants({MakeJitConstant("COMPRESSED_WEIGHTS_TERNARY", 1)});
            if (params.weights.GetDType() == WeightsType::UT1_5) {
                jit.AddConstants({MakeJitConstant("TERNARY_BASE3_PACKING", 1)});
            }
        }
